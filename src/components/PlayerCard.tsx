...
const colorsVariants = {
  red: {
    ...
    gradient: "w-full absolute bg-gradient-to-b from-red-800/90 via-red-800/100 to-black bottom-0 pb-10 border-t-2 border-yellow-600",
  },
  orange: {
    ...
    gradient: "w-full absolute bg-gradient-to-b from-orange-800/90 via-orange-800/100 to-black bottom-0 pb-10 border-t-2 border-yellow-600",
  },
  slate: {
    ...
    gradient: "w-full absolute bg-gradient-to-b from-slate-400/90 via-slate-400/100 to-black bottom-0 pb-10 border-t-2 border-yellow-600",
  },
  sky: {
    ...
    gradient: "w-full absolute bg-gradient-to-b from-sky-600/90 via-sky-600/100 to-black bottom-0 pb-10 border-t-2 border-yellow-600",
  },
  purple: {
    ...
    gradient: "w-full absolute bg-gradient-to-b from-purple-900/90 via-purple-900/100 to-black bottom-0 pb-10 border-t-2 border-yellow-600",
  },
  green: {
    ...
    gradient: "w-full absolute bg-gradient-to-b from-green-800/90 via-green-800/100 to-black bottom-0 pb-10 border-t-2 border-yellow-600",
  },
} as const
...
const draw = {
  ...
  full: (i: number) => {
    ...
    return {
      ...
      opacity: 1,
      transition: {
        pathLength: {delay, duration: 5, type: "spring", bounce: 0,repeat: Infinity},
...
export function PlayerCard(
  { player, teamLogo, teamColor } :
  {player: PlayerDto, teamLogo: string, teamColor: COLORS}
  ) {
  ...
  useEffect(() => {
    ...
    if(div)
      div.innerHTML = svgRole
  })

  return (
    <motion.li 
      key={player.id} 
      className='flex flex-col flex-wrap items-center w-60 h-[23rem] relative'
      variants={item}
      transition={{duration: 1}}
    >
      <motion.svg initial="zero" animate="full" width="100%" height="100%" preserveAspectRatio="none" viewBox="0.5 2 87 143" className="absolute z-10 stroke-yellow-600">
        <path 
      ...
      >
        <Score 
          scores={player.score} 
          className="absolute z-20 left-9 leading-none top-14 flex flex-col-reverse items-center gap-1"
        />
        ...
        <Image 
          ...
          height={300} 
          alt=""
          className="absolute opacity-25 -z-10 top-10"
        />
        ...
        <Image 
          ...
          alt=""
          className="mt-6 absolute motion-safe:animate-pulse"
        />
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 150 146.76" className="absolute w-0 h-0">
        ...
        <Image 
          ...
          className='object-contain aspect-auto mt-24 min-h-[170px] max-h-[175px]'
        />
        <div 
          className={`${colorsVariants[teamColor as keyof typeof colorsVariants].gradient}`}
        >
          <div className="flex 
            flex-col 
            justify-end 
          ...
          >
            ...
            <p className='capitalize text-shadow font-normal text-xs'>{player.playerName.toLowerCase()}</p>
            <div ref={roleDiv} className='[&>svg]:w-6 [&>svg]:h-6'>
            </div>
          </div>
        </div>
      </div>
