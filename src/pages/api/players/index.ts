...
export default async function handler(
  ...
  if (req.method === 'POST') {
    ...
    const pupp = async () => {
      ...
      await page.goto('https://lolesports.com/standings/cblol-brazil/cblol_2023_split_1/regular_season');

      const teamSelector = '.ranking'
      await page.waitForSelector(teamSelector)

      teamsRaw = await page.$$eval('.ranking', element => element.map(team => {
      ...
      for(let team of teamsRaw) {
        ...
        players = await page.$$eval('.card', element => element.map(playerCard => {
          ...
          const playerNick = playerInfo?.querySelector('.summoner-name')?.innerHTML
          const playerName = playerInfo?.querySelector('.real-name')?.innerHTML
